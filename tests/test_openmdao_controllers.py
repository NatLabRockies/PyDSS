...
from pydss.pyControllers.pyController import ControllerTypes, Create
...
def _controller_runner(controller):
    ...
    def run(voltage, active, reactive, time):
        ...
        problem.run_model()
        values = {name: float(np.asarray(problem.get_val(f"controller.{name}")).reshape(-1)[0])
...
def test_pv_voltage_ride_through_trips_and_reconnects():
    controller = PvVoltageRideThru(settings={
        "uv_2_pu": 0.5, "uv_2_ct_sec": 1.0, "reconnect_deadtime_sec": 2.0,
    })
...
def test_motor_stall_simple_multiplies_then_disconnects_and_reconnects():
    controller = MotorStallSimple(settings={
        "v_stall": 0.6, "p_fault": 3.0, "q_fault": 4.0,
        "t_protection": 1.0, "t_reconnect": 6.0,
    })
    run = _controller_runner(controller)
...
def test_dynamic_voltage_support_injects_or_absorbs_reactive_power():
    settings = {
        "Trv": 0.001, "Tinv": 0.001, "dbd1": -0.1, "dbd2": 0.1,
        "Kqv": 10.0, "kvar_max": 1.0, "kvar_min": -1.0, "kva": 10.0,
    }
...
def test_migrated_time_and_voltage_families_use_synthetic_commands_only():
    ...
    generator = GenController(settings={"u_min": 0.9, "u_db_min": 0.95, "u_db_max": 1.05, "u_max": 1.1})
    assert _run_declared_inputs(generator, {"measurement_voltage_pu": 0.9,
                                            "measurement_active_power": 4.0,
                                            "measurement_reactive_power": 0.0})["command_reactive_power"] > 0.0

    pv = PvController(settings={"uMin": 0.94, "uDbMin": 0.97, "uDbMax": 1.03, "uMax": 1.06})
    assert _run_declared_inputs(pv, {"measurement_voltage_pu": 1.1,
                                     "measurement_active_power": 2.0,
                                     "measurement_reactive_power": 0.0})["command_reactive_power"] < 0.0

    dynamic = PvDynamic(settings={"response_fraction": 0.5})
    first = dynamic.compute_commands({"measurement_active_power": 4.0, "measurement_reactive_power": 2.0,
                                      "measurement_voltage_pu": 1.0, "time_seconds": 0.0})
    dynamic.commit_step()
    second = dynamic.compute_commands({"measurement_active_power": 0.0, "measurement_reactive_power": 0.0,
                                       "measurement_voltage_pu": 1.0, "time_seconds": 1.0})
...
def test_migrated_stall_and_frequency_controllers_commit_state():
    for controller in (
        ...
        result = _run_declared_inputs(controller, {"measurement_voltage_pu": 0.5,
                                                    "measurement_active_power": 2.0,
    ...
    ride_through = PvFrequencyRideThru(settings={"f_lower": 59.0, "f_upper": 61.0, "trip_delay_sec": 1.0})
    first = ride_through.compute_commands({"measurement_frequency_hz": 58.0,
                                           "measurement_active_power": 2.0,
                                           "measurement_reactive_power": 1.0,
                                           "time_seconds": 0.0})
    assert first["command_enabled"] == 1.0
    ride_through.commit_step()
    second = ride_through.compute_commands({"measurement_frequency_hz": 58.0,
                                            "measurement_active_power": 2.0,
                                            "measurement_reactive_power": 1.0,
                                            "time_seconds": 1.1})
...
def test_migrated_boundary_storage_thermal_and_transformer_controllers():
    ...
    socket_result = _run_declared_inputs(socket_controller, {"measurement_active_power": 0.0,
    ...
    assert _run_declared_inputs(storage, {"measurement_active_power": 8.0,
                                          "measurement_reactive_power": 0.0})["command_active_power"] == 5.0

    thermostat = ThermostaticLoad(settings={"Tmin": 18.0, "Tmax": 20.0, "kw": 2.0,
                                             "initial_temperature": 19.0, "initial_on": True})
    assert _run_declared_inputs(thermostat, {"time_seconds": 0.0, "measurement_active_power": 2.0})["command_active_power"] == 2.0

    transformer = xfmrController(settings={"RPF locking": True})
    assert _run_declared_inputs(transformer, {"measurement_active_power": -1.0})["command_enabled"] == 0.0