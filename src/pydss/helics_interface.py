        ...
        for subscription in self.subscriptions:
            if subscription.model not in self.opendss_models:
                raise AssertionError(
                    f"The loaded OpenDSS model does not have an element define with the name {subscription}"
                )
            ...
            if subscription.subscribe:
                ...
                subscription.sub = helics.helicsFederateRegisterSubscription(
                    self.federate, subscription.id, subscription.unit
                )
        return self
                    ...
                    for ppty in k["Publish"]:
                        ...
                        pub_dict = {
                            ...
                            "pub": helics.helicsFederateRegisterGlobalTypePublication(
                                self.federate, name, TYPE_INFO[ppty], ""
                            ),
                        }
                    ...
                    if export_property["publish"]:
                        ...
                        for model_name, model_obj in filtered_models.items():
                            if object_type == "Buses":
                                name = "{}.Bus.{}.{}".format(
                                    self.federate_name, model_name, export_property["property"]
                                )
                            else:
                                name = "{}.{}.{}".format(
                                    self.federate_name, model_name, export_property["property"]
                            ...
                            pub_dict = {
                                ...
                                "pub": helics.helicsFederateRegisterGlobalTypePublication(
                                    self.federate, name, TYPE_INFO[export_property["property"]], ""
                                ),
                            }
                            publications.append(Publication.model_validate(pub_dict))
        self.publications = publications
...
class HelicsInterface:
    ...
    def __init__(
        ...
        if default:
            self.register_pub_sub_tags()

    def register_pub_sub_tags(self, pubs=None, subs=None):

        self._register_federate_subscriptions(subs)
        self._register_federate_publications(pubs)

        helics.helicsFederateEnterExecutingModeIterative(
            self._federate, helics.helics_iteration_request_iterate_if_needed
        )
    ...
    def _create_helics_federate(self):
        ...
        helics.helicsFederateInfoSetCoreInitString(self.fedinfo, "--federates=1")
        broker = self._settings.helics.broker
        broker_port = self._settings.helics.broker_port
        logger.info(
            "Connecting to broker @ {}".format(f"{broker}:{broker_port}" if broker_port else broker)
        )
        ...
        helics.helicsFederateInfoSetTimeProperty(
            self.fedinfo, helics.helics_property_time_delta, self._settings.helics.time_delta
        )
        helics.helicsFederateInfoSetIntegerProperty(
            self.fedinfo, helics.helics_property_int_log_level, self._settings.helics.logging_level
        )
        helics.helicsFederateInfoSetIntegerProperty(
            self.fedinfo,
            helics.helics_property_int_max_iterations,
            self._settings.helics.max_co_iterations,
        )
        self._federate = helics.helicsCreateValueFederate(
            self._settings.helics.federate_name, self.fedinfo
        )
        return

    def _register_federate_subscriptions(self, subscriptions: Subscriptions = None):
        """
        :param subs:
        :return:
        """
        if subscriptions is None:
            ...
            assert os.path.exists(subscription_file), (
                f"The following file does not exist: {subscription_file}"
            )
    ...
    def update_helics_subscriptions(self):
        for subscription in self.subscriptions.subscriptions:
            if subscription.subscribe:
                value = None
                ...
                logger.info(
                    'Value for "{}.{}" changed to "{}"'.format(
                        subscription.model, subscription.property, value
                    )
                )
    ...
    def _register_federate_publications(self, publications: Publications = None):
        if publications:
    ...
    def update_helics_publications(self):

        for publication in self.publications.publications:
            value = publication.object.get_value(publication.property)
    ...
    def request_time_increment(self):
        error = sum(
            [abs(sub.states[0] - sub.states[1]) for sub in self.subscriptions.subscriptions]
        )
        r_seconds = (
            self._dss_solver.get_total_seconds()
        ...
        if not self._settings.helics.iterative_mode:
            ...
            self.c_seconds, iteration_state = helics.helicsFederateRequestTimeIterative(
                self._federate, r_seconds, helics.helics_iteration_request_iterate_if_needed
            )

            logger.info(
                "Time requested: {} - time granted: {} error: {} it: {}".format(
    ...
    def __del__(self):
        ...
        helics.helicsFederateGetState(self._federate)
...
globals()["helics_interface"] = HelicsInterface
setattr(HelicsInterface, "registerPubSubTags", HelicsInterface.register_pub_sub_tags)
setattr(
    HelicsInterface,
    "_registerFederateSubscriptions",
    HelicsInterface._register_federate_subscriptions,
)
setattr(HelicsInterface, "updateHelicsSubscriptions", HelicsInterface.update_helics_subscriptions)
setattr(
    HelicsInterface,
    "_registerFederatePublications",
    HelicsInterface._register_federate_publications,
)
setattr(HelicsInterface, "updateHelicsPublications", HelicsInterface.update_helics_publications)
