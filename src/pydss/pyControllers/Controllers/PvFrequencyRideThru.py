...
class PvFrequencyRideThru(ControllerExplicitComponent):
    ...
    def input_specs(self) -> tuple[VariableSpec, ...]:
        return (VariableSpec("measurement_frequency_hz"), VariableSpec("measurement_active_power"),
    ...
    def compute_commands(self, inputs) -> Mapping[str, Any]:
        ...
        frequency = float(inputs["measurement_frequency_hz"])
        ...
        connected = self._connected and not (violation_start is not None and time - violation_start >= delay)
        self._pending_state = (connected, violation_start)
        return {"command_active_power": float(inputs["measurement_active_power"]) if connected else 0.0,
                "command_reactive_power": float(inputs["measurement_reactive_power"]) if connected else 0.0,
                "command_enabled": float(connected), "diagnostic_residual": float(connected != self._connected)}
