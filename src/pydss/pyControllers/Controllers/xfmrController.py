...
class xfmrController(ControllerExplicitComponent):
    ...
    def compute_commands(self, inputs) -> Mapping[str, Any]:
        ...
        reverse_power_locking = bool(settings.get("rpf_locking", settings.get("RPF locking", False)))
        power = float(inputs["measurement_active_power"])
        locked = reverse_power_locking and power < 0.0
        self._pending_locked = locked
        return {"command_enabled": float(not locked), "diagnostic_residual": float(locked != self._locked)}
