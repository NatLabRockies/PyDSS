...
class SocketController(ControllerExplicitComponent):
    ...
    def compute_commands(self, inputs) -> Mapping[str, Any]:
        ...
        active = float(settings.get("active_power", inputs["measurement_active_power"]))
        reactive = float(settings.get("reactive_power", inputs["measurement_reactive_power"]))
        self._pending = (active, reactive)
        return {"command_active_power": active, "command_reactive_power": reactive,
