...
import pydss.defaults as default_settings
...
def bytestream_decode(s):
    ...
    for i, line in enumerate(lines):
        if line.startswith(filter):
            line = line.replace(filter, b"").replace(b'"', b"").decode("ascii")
            if "; filename=" in line:
                data = line.replace("; filename=", ":").split(":")
...
def restructure_dictionary(d):
    global master_dict
    pydss_settings = {}
    path = default_settings.__path__._path[0]
        ...
        for key, valid_entries in master_dict.items():
            if k in valid_entries:
                if key not in pydss_settings:
                    pydss_settings[key] = {}
                if i == "False" or i == "false" or not i:
                    i = False
                elif i == "True" or i == "true" or i:
                    i = True
                else:
                    try:
                        i = int(i)
    ...
    defaults["Project"]["Scenarios"] = [{"name": d["Active Scenario"], "post_process_infos": []}]
