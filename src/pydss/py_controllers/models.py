...
from pydss.py_controllers.enumerations import (
    CategoryI,
    CategoryII,
    CategoryIII,
    PvStandard,
...
class BaseControllerModel(BaseModel):
    ...
    def reject_legacy_controller_settings(cls, values):
        if isinstance(values, dict):
            legacy = sorted(
                key for key in values if key in {"Control1", "Control2", "Control3", "Priority"}
            )
            if legacy:
                raise ValueError(
...
class PvVoltageRideThruModel(BaseControllerModel):
    ...
    ride_through_category: Annotated[
        RideThroughCategory,
        Field(
            RideThroughCategory.CATEGORY_I,
            description="PV ride-through category fot the inverter (see IEEE 1547-2018 std for more information).",
        ),
    ]
    ov_2_pu: Annotated[
        float,
    ...
    ov_2_ct_sec: Annotated[
        float,
        Field(
            CategoryI.OV2_CT_SEC.value,
            description="Trip duration setting if the upper bound of the over-voltage region is violated.",
        ),
    ]
    ov_1_pu: Annotated[
        float,
    ...
    ov_1_ct_sec: Annotated[
        float,
        Field(
            CategoryI.OV1_CT_SEC.value,
            description="Trip duration setting if the lower bound of the over-voltage region is violated.",
        ),
    ]
    uv_1_pu: Annotated[
        float,
    ...
    uv_1_ct_sec: Annotated[
        float,
        Field(
            CategoryI.UV1_CT_SEC.value,
            description="Trip duration setting if the upper bound of the under-voltage region is violated.",
        ),
    ]
    uv_2_pu: Annotated[
        float,
    ...
    reconnect_pmax_time_sec: Annotated[
        ...
        Field(
            300.0,
            ge=0.0,
            description="Reconnect after a trip event. PV system will connect back once this time has elapsed and the system voltage is within bounds.",
        ),
    ]
    permissive_operation: Annotated[
        ...
        Field(
            ...
            description="Defines behavior of the system within the 'permissive operation' region. (see IEEE 1547-2018 std for more information).",
        ),
    ]
    may_trip_operation: Annotated[
        MayTripOperation,
        Field(
            MayTripOperation.TRIP,
            description="Defines behavior of the system within the 'may trip' region. (see IEEE 1547-2018 std for more information).",
        ),
    ]
    multiple_disturdances: Annotated[
        MultipleDisturbances,
        Field(
            MultipleDisturbances.TRIP,
            description="Defines behavior of the system after multiple disturbances. (see IEEE 1547-2018 std for more information).",
        ),
    ]
    ...
    def update_settings(self) -> "PvVoltageRideThruModel":
        ...
        self.ov_2_pu = (
            CategoryI.OV2_PU.value
            if cat1
            else CategoryII.OV2_PU.value
            if cat2
            else CategoryIII.OV2_PU.value
        )
        self.ov_1_pu = (
            CategoryI.OV1_PU.value
            if cat1
            else CategoryII.OV1_PU.value
            if cat2
            else CategoryIII.OV1_PU.value
        )
        self.uv_2_pu = (
            CategoryI.UV2_PU.value
            if cat1
            else CategoryII.UV2_PU.value
            if cat2
            else CategoryIII.UV2_PU.value
        )
        self.uv_1_pu = (
            CategoryI.UV1_PU.value
            if cat1
            else CategoryII.UV1_PU.value
            if cat2
            else CategoryIII.UV1_PU.value
        )
        self.ov_2_ct_sec = (
            CategoryI.OV2_CT_SEC.value
            if cat1
            else CategoryII.OV2_CT_SEC.value
            if cat2
            else CategoryIII.OV2_CT_SEC.value
        )
        self.ov_1_ct_sec = (
            CategoryI.OV1_CT_SEC.value
            if cat1
            else CategoryII.OV1_CT_SEC.value
            if cat2
            else CategoryIII.OV1_CT_SEC.value
        )
        self.uv_2_ct_sec = (
            CategoryI.UV2_CT_SEC.value
            if cat1
            else CategoryII.UV2_CT_SEC.value
            if cat2
            else CategoryIII.UV2_CT_SEC.value
        )
        self.uv_1_ct_sec = (
            CategoryI.UV1_CT_SEC.value
            if cat1
            else CategoryII.UV1_CT_SEC.value
            if cat2
            else CategoryIII.UV1_CT_SEC.value
        )
...
class PvSmartController(BaseControllerModel):
    ...
    pct_p_cutin: Annotated[
        ...
        Field(
            ...
            ge=0.0,
            le=100.0,
            description="Percentage of kVA rating of inverter. When the inverter is OFF, the power from the system must be greater than this for the inverter to turn on",
        ),
    ]
    pct_p_cutout: Annotated[
        ...
        Field(
            ...
            ge=0.0,
            le=100.0,
            description="Percentage of kVA rating of inverter. When the inverter is ON, the inverter turns OFF when the power from the array drops below this value.",
        ),
    ]
    ...
    pf_min: Annotated[
        ...
        Field(
            ...
            ge=0.0,
            le=1.0,
            description="Minimum allowable powerfactor for the system. 'enable_pf_limit' should be enable for the constraint to be implemented.",
        ),
    ]
...
class MotorStallSimpleSettings(BaseControllerModel):
    ...
    v_stall: Annotated[
        float,
        Field(
            0.55, ge=0.53, le=0.58, description="Per unit voltage below which the motor will stall."
        ),
    ]
    t_protection: Annotated[
        float,
        Field(
            0.95, ge=0.0, le=15.0, description="Time [sec] after stall the motor will disconnect."
        ),
    ]
...
class MotorStallSettings(BaseControllerModel):
    ...
    t_th: Annotated[
        ...
        Field(
            4.0,
            description="Varies based on manufacturer and external factors - sensitivity analysis required",
        ),
    ]
    f_rst: Annotated[
        float,
        Field(
            0.2,
            description="Captures diversity in load; also based on testing (fraction of motors capable of restart).",
        ),
    ]
    lf_adj: Annotated[
        float,
    ...
    v_stall: Annotated[
        float,
        Field(
            0.55, ge=0.45, le=0.60, description="Stall voltage (range) based on laboratory testing"
        ),
    ]
    v_break: Annotated[
        float,
    ...
    r_stall_pu: Annotated[
        ...
        Field(
            0.100,
            description="Based on laboratory testing results of residential air-conditioners.",
        ),
    ]
...
class PvControllerModel(BaseControllerModel):
    ...
    voltage_calc_mode: Annotated[
        VoltageCalcModes,
        Field(VoltageCalcModes.MAX, description="Voltage values used for calculations."),
    ]
