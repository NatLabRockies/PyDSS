...
class StorageController(ControllerExplicitComponent):
    ...
    def compute_commands(self, inputs) -> Mapping[str, Any]:
        ...
        measured = self.input_value(inputs, "measurement_active_power")
        ...
        if control in {"rt", "realtime", "real_time"}:
            ...
            index = (
                min(
                    ...
                    max(
                        0,
                        int(
                            self.input_value(inputs, "time_seconds")
                            / max(float(settings.get("schedule_period_sec", 3600.0)), 1.0)
                        ),
                    ),
                )
                if schedule
            ...
            target = measured - max(0.0, measured - upper) * float(
                settings.get("damp_coef", settings.get("DampCoef", 1.0))
            )
        else:
            target = float(settings.get("active_power", measured))
        self._pending_active = target
        return {
            "command_active_power": target,
