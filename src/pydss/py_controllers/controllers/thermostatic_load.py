...
class ThermostaticLoad(ControllerExplicitComponent):
    ...
    def setup(self) -> None:
        ...
        self._temperature = float(
            settings.get(
                "initial_temperature",
                (float(settings.get("Tmin", 18.0)) + float(settings.get("Tmax", 30.0))) / 2.0,
            )
        )
    ...
    def compute_commands(self, inputs) -> Mapping[str, Any]:
        ...
        ambient = 30.0 + 10.0 * math.sin(
            self.input_value(inputs, "time_seconds") * 2.0 * math.pi / 86400.0
        )
        rated = float(settings.get("kw", self.input_value(inputs, "measurement_active_power")))
        ...
        return {
            "command_active_power": rated if on else 0.0,
            "diagnostic_residual": temperature - self._temperature,
        }
