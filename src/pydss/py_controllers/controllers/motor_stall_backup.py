...
class MotorStallBackup(ControllerExplicitComponent):
    ...
    def compute_commands(self, inputs) -> Mapping[str, Any]:
        ...
        enabled = self.input_value(inputs, "measurement_voltage_pu") >= float(
            settings.get("v_stall", 0.55)
        )
        ...
        return {
            "command_active_power": active if enabled else 0.0,
            "command_reactive_power": reactive if enabled else 0.0,
            "command_enabled": float(enabled),
            "diagnostic_residual": float(state != self._state),
        }
