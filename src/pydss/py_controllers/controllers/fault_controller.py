...
class FaultController(ControllerExplicitComponent):
    ...
    def compute_commands(self, inputs) -> Mapping[str, Any]:
        ...
        start = float(
            settings.get("start_time_seconds", settings.get("Fault start time (sec)", 0.0))
        )
        duration = float(
            settings.get("duration_seconds", settings.get("Fault duration (sec)", 0.0))
        )
        enabled = start <= time < start + duration
        self._pending_fault_enabled = enabled
        return {
            "command_enabled": float(enabled),
            "diagnostic_residual": float(enabled != self._fault_enabled),
        }
